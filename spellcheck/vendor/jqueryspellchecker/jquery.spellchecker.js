	...
	SpellChecker.prototype = {
		...
		replace : function(replace) {
			...
			if(typeof tinyMCE !== 'undefined'){
				if (tinyMCE.get(this.$domObj.attr('id'))) {
					tinyMCE.execCommand('mceRemoveControl', false, this.$domObj.attr('id'));
					mceFlag = 1;
				}
			}
			switch(this.type) {
			...
			if (mceFlag) {
				tinyMCE.execCommand('mceAddControl', false, this.$domObj.attr('id'));
			} 
		},
